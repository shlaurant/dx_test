...
void FBXLoader::Import(const wstring &path) {
    ...
    _resourceDirectory = fs::path(path).parent_path().wstring() + L"\\" +
                         fs::path(path).filename().stem().wstring() + L".fbm";

    _importer = FbxImporter::Create(_manager, "");

    string strPath = ws2s(path);
    auto r0 = _importer->Initialize(strPath.c_str(), -1,
...
void FBXLoader::LoadMesh(FbxMesh *mesh) {
    ...
    for (int32_t i = 0; i < triCount; i++) {
        for (int32_t j = 0; j < 3; j++) {
            ...
            GetUV(mesh, &meshInfo, controlPointIndex,
                  mesh->GetTextureUVIndex(i, j));

            vertexCounter++;
        }

        const uint32_t subsetIdx = geometryElementMaterial->GetIndexArray().GetAt(
                i);
...
void FBXLoader::LoadMaterial(FbxSurfaceMaterial *surfaceMaterial) {
    ...
    material.diffuse = GetMaterialData(surfaceMaterial,
                                       FbxSurfaceMaterial::sDiffuse,
                                       FbxSurfaceMaterial::sDiffuseFactor);
    material.ambient = GetMaterialData(surfaceMaterial,
                                       FbxSurfaceMaterial::sAmbient,
                                       FbxSurfaceMaterial::sAmbientFactor);
    material.specular = GetMaterialData(surfaceMaterial,
                                        FbxSurfaceMaterial::sSpecular,
                                        FbxSurfaceMaterial::sSpecularFactor);

    material.diffuseTexName = GetTextureRelativeName(surfaceMaterial,
                                                     FbxSurfaceMaterial::sDiffuse);
    material.normalTexName = GetTextureRelativeName(surfaceMaterial,
                                                    FbxSurfaceMaterial::sNormalMap);
    material.specularTexName = GetTextureRelativeName(surfaceMaterial,
                                                      FbxSurfaceMaterial::sSpecular);

    _meshes.back().materials.push_back(material);
}

void FBXLoader::GetNormal(FbxMesh *mesh, FbxMeshInfo *container, int32_t idx,
...
void FBXLoader::GetTangent(FbxMesh *mesh, FbxMeshInfo *meshInfo, int32_t idx,
                           int32_t vertexCounter) {
    ...
    } else if (tangent->GetMappingMode() ==
               FbxGeometryElement::eByControlPoint) {
        if (tangent->GetReferenceMode() == FbxGeometryElement::eDirect)
            tangentIdx = idx;
        else
...
void FBXLoader::GetUV(FbxMesh *mesh, FbxMeshInfo *meshInfo, int32_t idx,
                      int32_t uvIndex) {
...
Vector4 FBXLoader::GetMaterialData(FbxSurfaceMaterial *surface,
...
wstring FBXLoader::GetTextureRelativeName(FbxSurfaceMaterial *surface,
    ...
    if (textureProperty.IsValid()) {
        ...
        if (1 <= count) {
            FbxFileTexture *texture = textureProperty.GetSrcObject<FbxFileTexture>(
                    0);
            if (texture)
...
std::vector<std::wstring> FBXLoader::textures() {
...
void FBXLoader::LoadAnimationData(FbxMesh *mesh, FbxMeshInfo *meshInfo) {
    ...
    for (int32_t i = 0; i < skinCount; i++) {
        FbxSkin *fbxSkin = static_cast<FbxSkin *>(mesh->GetDeformer(i,
                                                                    FbxDeformer::eSkin));
            ...
            if (FbxSkin::eRigid == type || FbxSkin::eLinear) {
                ...
                for (int32_t j = 0; j < clusterCount; j++) {
                    ...
                    for (int32_t k = 0; k < animCount; k++)
                        LoadKeyframe(k, mesh->GetNode(), cluster,
                                     matNodeTransform, boneIdx, meshInfo);
                }
            }
        }
    }
...
void FBXLoader::LoadBoneWeight(FbxCluster *cluster, int32_t boneIdx,
...
void FBXLoader::LoadOffsetMatrix(FbxCluster *cluster,
                                 const FbxAMatrix &matNodeTransform,
...
void
FBXLoader::LoadKeyframe(int32_t animIndex, FbxNode *node, FbxCluster *cluster,
    ...
    FbxLongLong startFrame = _animClips[animIndex]->startTime.GetFrameCount(
            timeMode);
    FbxLongLong endFrame = _animClips[animIndex]->endTime.GetFrameCount(
            timeMode);
    ...
    for (FbxLongLong frame = startFrame; frame < endFrame; frame++) {
        ...
        FbxAMatrix matTransform = matFromNode.Inverse() *
...
FbxAMatrix FBXLoader::GetTransform(FbxNode *node) {
    const FbxVector4 translation = node->GetGeometricTranslation(
            FbxNode::eSourcePivot);
    const FbxVector4 rotation = node->GetGeometricRotation(
            FbxNode::eSourcePivot);
